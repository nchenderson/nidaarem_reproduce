...
NIDAARAMNI <- RNIDAARAMNI <- DAARAMNI <- RDAARAMNI <- SQNI <- RENESTNI <- NIDAAREMNI <- matrix(0, nrow=nreps, ncol=n.lambda)
NESTNI <- PGDNI <- MPENI <- SNIDAARAMNI <- SDAARAMNI <- matrix(0, nrow=nreps, ncol=n.lambda)
NIDAARAMObj <- RNIDAARAMObj <- DAARAMObj <- RDAARAMObj <- SQObj <- RENESTObj <- NIDAAREMObj <- matrix(0, nrow=nreps, ncol=n.lambda)
NESTObj <- PGDObj <- MPEObj <- SNIDAARAMObj <- SDAARAMObj <- matrix(0, nrow=nreps, ncol=n.lambda)
RNIDAARAMConv <- NIDAARAMConv <- RDAARAMConv <- DAARAMConv <- SQConv <- RENESTConv <- NIDAAREMConv <- matrix(0, nrow=nreps, ncol=n.lambda)
NESTConv <- PGDConv <- MPEConv <- SNIDAARAMConv <- SDAARAMConv <- matrix(0, nrow=nreps, ncol=n.lambda)
NIDAARAMTime <- RNIDAARAMTime <- DAARAMTime <- RDAARAMTime <- SQTime <- RENESTTime <- NIDAAREMTime <- matrix(0, nrow=nreps, ncol=n.lambda)
...
for(j in 1:nreps) {
  for(k in 1:n.lambda) {
    ...
    ans.nidaaram.resid.time <- system.time(ans.nidaaram.resid <- daarem.lasso(par=beta.init, X=XX, y=yy, lambda=lam, stplngth=stp, nesterov.init=TRUE, 
                                                                              control=list(tol=tols, order=5, maxiter=ni, mon.tol=c(.95,0), objfn.check=FALSE)))
    ...
    RNIDAARAMNI[j,k] <- ans.nidaaram.resid$fpevals
    ...
    RNIDAARAMObj[j,k] <- ans.nidaaram.resid$value.objfn
    ...
    RNIDAARAMConv[j,k] <- ans.nidaaram.resid$convergence
    ...
    RNIDAARAMTime[j,k] <- ans.nidaaram.resid.time[3]
...
NESTNI, PGDNI, MPENI, SNIDAARAMNI, SDAARAMNI, NIDAAREMNI, NIDAARAMObj, RNIDAARAMObj, DAARAMObj, 
RDAARAMObj, SQObj, RENESTObj, NESTObj, PGDObj, MPEObj, SNIDAARAMObj, SDAARAMObj, NIDAAREMObj,
RNIDAARAMConv, NIDAARAMConv, RDAARAMConv, DAARAMConv, SQConv, RENESTConv, 
NESTConv, PGDConv, MPEConv, SNIDAARAMConv, SDAARAMConv, NIDAAREMConv, 
NIDAARAMTime, RNIDAARAMTime, DAARAMTime, RDAARAMTime, SQTime, RENESTTime,
NESTTime, PGDTime, MPETime, SNIDAARAMTime, SDAARAMTime, NIDAAREMTime,
file="~/Documents/NIDAAREM/SimulationResults/LassoSmallnBigp1a.RData")
