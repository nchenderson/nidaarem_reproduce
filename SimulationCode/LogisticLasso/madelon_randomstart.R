...
## the "madelon" data. Only two values of the penalty terms are 
## considered and 50 random starting values are drawn
...
lambda.seq <- lambda.seq[c(2,6)]
## This only uses lambda=67.6 and lambda=8.7
...
for(k in 1:n.lambda) {
  ...
  for(j in 1:n.reps) {
     ...
     ## Draw starting values from N(0, 1).
     beta.init <- rnorm(p)
...
## Save results:
fname <- "SimulationResults/LogisticLasso/MadelonFixedLambdas.RData"
