...
## Note that the functions MatrixCompleteUpdate and MatrixCompleteObj
## are made available by loading the nidaarem package
...
for(k in 1:2) {
    ...
    for(h in 1:nreps) {
        ...
        ans.si.time <- system.time(ans.si <- softImpute(XNA, rank.max=942, lambda=lambda.star, thresh=1e-12, maxit=maxit))
    ...
    if(lambda.star==20) {
        fname <- "SimulationResults/MatrixCompletion/MatrixCompleteSimFixedLambda20.RData"
    } else if(lambda.star==200){
        fname <- "SimulationResults/MatrixCompletion/MatrixCompleteSimFixedLambda200.RData"
    }
    save(DAARAMNI, NIDAARAMNI, RNIDAARAMNI, SQNI, RENESTNI, NESTNI, PGDNI,
         DAARAMObj, NIDAARAMObj, RNIDAARAMObj, SQObj, RENESTObj, NESTObj, PGDObj, SIObj,
         DAARAMTime, NIDAARAMTime, RNIDAARAMTime, SQTime, RENESTTime, NESTTime, PGDTime, SITime,
         DAARAMConv, NIDAARAMConv, RNIDAARAMConv, SQConv, RENESTConv, NESTConv, PGDConv,
         file=fname)
}
