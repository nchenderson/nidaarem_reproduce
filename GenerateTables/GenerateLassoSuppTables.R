...
### Code to generate Tables S1-S5 for supplement
#### These are the lasso simulations for fixed lambda
...
count <- 1
for(design_setting in c(1,2,4,5,6)) {
  ...
  fname <- paste("Tables/TableS", count, "LassoSimResults.RData", sep="")
  save(LassoResultsTable, file=fname)
  count <- count + 1
}
